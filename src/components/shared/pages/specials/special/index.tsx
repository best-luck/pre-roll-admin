...
export default function Special({special, type}: {special: SpecialType, type?: string}) {
  return (
    <div className={`flex justify-center items-center flex-col ${type==="view"?"w-full":"specials-grid"}`}>
      <div className={`bg-cover bg-no-repeat bg-center rounded-[20px] p-5 text-white h-[200px]`} style={{backgroundImage: `linear-gradient(90deg, rgba(11, 31, 50, 0.38) 0%, rgb(11, 31, 50) 100%), url(${SPECIAL_IMAGE_HOST}${special.menuDisplayConfiguration.image})`}}>
        {
        type==="view" ?
          <>
